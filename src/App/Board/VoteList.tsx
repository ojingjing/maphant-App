...
import { listVoteBoard, searchArticle } from "../../Api/board";
...
const VoteDetailList: React.FC = () => {
  const params = useRoute().params as { boardType: BoardType };
  const boardType = params?.boardType;
  const [boardData, setboardData] = useState<VoteBoard[]>([]);
  const [refreshing, setRefreshing] = useState(false);
  const navigation = useNavigation<NavigationProp<NavigationProps>>();
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [searchResults, setSearchResults] = useState<VoteBoard[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      const data = await listVoteBoard(boardType.id, 1, 50);
      if (data.data) {
        setboardData(data.data.list as VoteBoard[]);
      }
    } catch (err) {
      console.log(err);
    } finally {
  ...
  const handleSearch = async (searchText: string) => {
    ...
    try {
      ...
      setSearchResults(data.data as VoteBoard[]);
